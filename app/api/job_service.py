...
#                 logger.info(f"Saving progress {progress_value}% for job {job_id}")
...
#             logger.debug(
#                 f"Saved job status for {job_id}: status={metadata.get('status')}, progress={metadata.get('progress')}"
#             )
#         except Exception as e:
...
#                 # Run this in background or thread if you want it strictly non-blocking
#                 s3_manager.sync_job_to_s3(job_id)

#             # Persist status and message to metadata file
#             self._save_job_status(job_id)
